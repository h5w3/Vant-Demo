<template>
	<div class="main">
		...
		<div id="cart">
			...
			<div id="list" style="padding-top:48px;padding-bottom: 0px;">
		    	<ul class="cart-ul">
				  <van-list
				  v-model="loadingShow"
				  :finished="finished"
				  ...
				  <li v-for="item in list">
					...
					<div class="cart-opt">
						<van-button size="mini" type="danger" 
							@click="getTkl('购买',item.title,item.coupon_click_url,item.click_url,item.pict_url)">购买</van-button>
			  		</div>
				  </li>
				  </van-list>
				</ul>
...
<script>
	...
	import { getShareCartItemListService, addShopClickNumService, getTklService, delCartItemService } from '@/api/service';
	...
	export default {
		...
		created(){
			this.getShareCartItemList();
			this.baseUrl = location.href.split("/cart")[0];
		},
		methods:{
			async getShareCartItemList(){
        		...
        		const res = await getShareCartItemListService(data);
        	...
        	async getTkl(t,title,coupon_click_url,click_url,pict_url){
		   		this.waitShow = true;
		   		//this.countPreBuyNum();
